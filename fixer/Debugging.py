...
def check_one_parameter_changing(original_parameters):
    scenario_path = "/home/xdzhang/ABLE/test_cases/buggy_scenarios/T-Junction/Green_Light_Rush_Cause_Collisions2.json"
    with open(scenario_path, "r") as f:
        scenario_json = json.load(f)
    ...
    parameter = "still_obstacle_speed_threshold"

    apply_change_to_ads(original_parameters)
    for _ in range(5):
        loop = asyncio.get_event_loop()
        flag, degree = loop.run_until_complete(law_complying_degree(scenario_json, spec, directory=log_directory))
        print("Before changing, {}".format(degree))

    exit(789)
    # Change
    changed_value = original_parameters[parameter] * 0.5
    changed_configuration = copy.deepcopy(original_parameters)
    changed_configuration[parameter] = changed_value
    apply_change_to_ads(changed_configuration)
    for _ in range(5):
        flag, degree = loop.run_until_complete(law_complying_degree(scenario_json, spec, directory=log_directory))
        print("After changing, {}".format(degree))
    ...
    for root, _, data_files in os.walk(data_path):
        for data_file in data_files:
            ...
            with open(os.path.join(root, data_file)) as f:
                ...
                robustness = monitor.continuous_monitor2(spec)
                print('Print {}: {}'.format(os.path.join(root, data_file), robustness))
...
num_population = 20
num_generation = 20
log_directory = "/home/xdzhang/test_log"
